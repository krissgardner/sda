...
void removeEndl(char *str) { str[strcspn(str, "\r\n")] = '\0'; }
...
void Movie_print(Movie *movie, FILE *fout) {
  fprintf(fout, "%-70s %-6hd %-6hd %-16s %-3s\n", movie->title, movie->year,
          movie->duration, categs.array[movie->category],
...
void Movie_partition(Movie **movies, int low, int high, int *i, int *j) {
  ...
  if (high - low <= 1) {
    if (categs.sortedIndex[movies[high]->category] <
        categs.sortedIndex[movies[low]->category]) {
  ...
  while (mid <= high) {
    ...
    } else if (categs.sortedIndex[movies[mid]->category] >
               categs.sortedIndex[pivot]) {
...
void Data_heapify(Data data, int i) {
  ...
  if (l < data.len &&
      strcmp(data.movies[l]->title, data.movies[largest]->title) < 0) {
    largest = l;
  }

  if (r < data.len &&
      strcmp(data.movies[r]->title, data.movies[largest]->title) < 0) {
    largest = r;
  }
...
int main() {
  ...
  while (fgets(line, LINEMAX, fin)) {
    ...
    for (char *token = strtok(line, "\t"), i = 0; token != NULL;
         token = strtok(NULL, "\t"), i++) {
      // printf("%s\n", token);
      switch (i) {
      case 0: {
      ...
      case 2: {
        // No need to handle realloc failure because there will always be less
        // memory
        strcpy(crt->title, token);
        crt->title = (char *)realloc(crt->title,
                                     (strlen(crt->title) + 1) * sizeof(char));
        break;
      }
      case 3: {
        // strcpy(crt->category, token);
        // crt->category = (char*)realloc(crt->category, (strlen(crt->category)
        // + 1) * sizeof(char));
        int found = 0;
        for (int i = 0; i < categs.len; i++) {
        ...
        if (!found) {
          ...
          // Add new category
          char **temp =
              (char **)realloc(categs.array, (categs.len + 1) * sizeof(char *));
          if (temp == NULL) {
            // Flag error
            memoryFail = 1;
          }
